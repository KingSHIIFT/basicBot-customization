...
bot.commands.headCommand = {
            command: 'head',  //The command to be called. With the standard command literal this would be: !head
            rank: 'bouncer', //Minimum user permission to use the command
            type: 'exact', //Specify if it can accept variables or not (if so, these have to be handled yourself through the chat.message
            functionality: function (chat, cmd) {
                if (this.type === 'exact' && chat.message.length !== cmd.length) return void (0);
    ...
    localStorage.setItem("basicBotsettings", JSON.stringify({
        ...
        motdEnabled: true,
        motdInterval: 5,
        motd: "Welcome to Rap & Hip-Hop Nation",
        filterChat: true,
